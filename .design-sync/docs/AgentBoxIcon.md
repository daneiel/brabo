---
category: Icones
keywords: [agente, bot, caixa]
---

# AgentBoxIcon

Ícone outline do set do Brabo: desenha um agente dentro de uma caixa. Na app aparece em identidade de agente no painel do time.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.AgentBoxIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <AgentBoxIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<AgentBoxIcon size={16} aria-label="um agente dentro de uma caixa" />
```
