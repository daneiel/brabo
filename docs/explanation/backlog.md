...
| Decompose `SessionPage.tsx` / `ProjectSettingsTab.tsx` | G | `SessionPage.tsx` is 169 KiB with 25 test files importing it; `ProjectSettingsTab.tsx` is 90 KiB | **IN PROGRESS** ([ADR 0122](../adr/0122-sessionpage-dividido-em-cinco-prs.md)) — `SessionPage.tsx` is now a tracked row in [architecture.md's debt table](../architecture.md#divida-tecnica), declared as a 5-PR mechanical decomposition executed one PR at a time (each merged before the next starts, since the file is under active churn). PR 1/5 done: pure timeline/turn helpers → `apps/web/src/lib/session-timeline.ts`. PR 2/5 done: `StorySlide` → `apps/web/src/routes/StorySlide.tsx`. PR 3/5 done: `StructuredQuestionCard` (plus its private helper `permiteOutra`) → `apps/web/src/routes/StructuredQuestionCard.tsx`. PR 4/5 done: backlog-tree helpers (`urlDaPr`/`vinculoDeBacklog`/`montarArvoreDeBacklog`/`totalDeDescendentes`) → `apps/web/src/lib/session-backlog-tree.ts`, and `ItemDeBacklog` + `ContextAside` (the whole right-hand sidebar) → `apps/web/src/routes/ContextAside.tsx`. One PR remains (a `useSessionReadiness` hook). `ProjectSettingsTab.tsx` stays a separate, unscheduled scope decision |
