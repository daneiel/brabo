...
Ícone outline do set do Brabo: desenha um balão de conversa. Na app aparece em a aba de chat da sessão.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.ChatIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <ChatIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<ChatIcon size={16} aria-label="um balão de conversa" />
```
