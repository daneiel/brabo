...
| FASE 30 (sessão 8) — o agente espera o primeiro projeto, e a chave nasce no terminal | ADR 0155, RN-550/551 |
...
remover nem renomear nenhuma e sem tocar `Restart=on-abnormal`. A sessão 8
(RN-550/551) entregou as duas peças que faltavam DO LADO DO AGENTE:
ele FICA DE PÉ com zero projetos e reconsulta a lista (a instalação nova deixa
de terminar sem agente), e `brabo-runner device-key create|finish` gera o par
Ed25519 NA MÁQUINA. O que segue sem existir é a ROTA que registra a pública de
MÁQUINA e o `install.sh` que encadeia os três comandos (sessão 6): o modo novo
só é exercitável com chave registrada à mão, e o `install --machine` não sabe
distinguir chave de máquina de chave de projeto — em disco são o mesmo arquivo,
e a recusa só chega no primeiro boot.
...
- Chave de dispositivo órfã (aba fechada no meio do fluxo da RN-473) é INERTE
  ...
  continua declarada. O MATERIAL de uma chave de máquina já nasce no terminal
  desde a RN-551 (`brabo-runner device-key create`), mas ninguém REGISTRA a
  pública ainda: a rota de chave de máquina e o `install.sh` que a chama (ADR
  0155 ponto 4) são outra sessão da FASE 30 — a rota nasce no PR que tiver o
  primeiro chamador real. Desde a RN-548 o web CONSOME essa
