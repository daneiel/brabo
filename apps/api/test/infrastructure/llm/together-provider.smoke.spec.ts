    ...
    it(
      ...
      async () => {
        ...
        const [owner] = await db
          .insert(users)
          .values({ keycloakSub: 'sub-together-smoke', email: 'together-smoke@brabo.dev' })
          .returning();
        const [workspace] = await db
          .insert(workspaces)
          .values({ name: 'smoke', slug: 'smoke-together', createdBy: owner.id })
          .returning();
        ...
        // 1) Cadastro — teste de conexão real (GET /v1/models, status-only).
