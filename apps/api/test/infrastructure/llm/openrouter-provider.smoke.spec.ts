...
import { models, projects, sessions, tokenUsage, users, workspaces } from '../../../src/db/schema';
    ...
    it(
      ...
      async () => {
        ...
        const [owner] = await db
          .insert(users)
          .values({ keycloakSub: 'sub-openrouter-smoke', email: 'openrouter-smoke@brabo.dev' })
          .returning();
        const [workspace] = await db
          .insert(workspaces)
          .values({ name: 'smoke', slug: 'smoke-openrouter', createdBy: owner.id })
          .returning();
        ...
        // 1) Cadastro — passa pelo teste de conexão REAL (GET /key) antes de
        //    cifrar/persistir. Chave inválida derruba o teste aqui, cedo.
