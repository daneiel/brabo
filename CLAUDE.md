...
- `Docker › Destroy` não remove `ollama`/`ollama-model-loader` quando estão
  de pé sob `profiles: ["local-llm"]` — `down` sem `--profile` ignora
  contêiner de perfil inativo e pode deixar a rede presa; fora do escopo
  do pedido que criou o perfil (ADR 0114)
