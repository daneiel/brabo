---
category: Icones
keywords: [local, repositório, offline]
---

# LocalRepoIcon

Ícone outline do set do Brabo: desenha um monitor de repositório local. Na app aparece em provider local, sem serviço remoto.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.LocalRepoIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <LocalRepoIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<LocalRepoIcon size={16} aria-label="um monitor de repositório local" />
```
