---
category: Icones
keywords: [layout, sidebar, frontend, interface]
---

# LayoutSidebarIcon

Ícone outline do set do Brabo: desenha um layout com barra lateral. Na app aparece em o agente Dev Frontend e alternar a sidebar.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.LayoutSidebarIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <LayoutSidebarIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<LayoutSidebarIcon size={16} aria-label="um layout com barra lateral" />
```
