...
- \`main\` — produção. Só recebe merge de \`rc\`.
- \`rc\` (release candidate) — recebe merge de \`qa\` quando uma release
  está pronta pra validação final.
- \`qa\` — recebe merge de \`dev\` pra validação antes de virar release
  candidate.
