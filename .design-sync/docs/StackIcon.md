---
category: Icones
keywords: [arquitetura, camadas, backlog, pilha]
---

# StackIcon

Ícone outline do set do Brabo: desenha camadas empilhadas. Na app aparece em o Arquiteto, backlog e execução.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.StackIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <StackIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<StackIcon size={16} aria-label="camadas empilhadas" />
```
