---
category: Icones
keywords: [hipótese, psicólogo, análise, parecer]
---

# HypothesisIcon

Ícone outline do set do Brabo: desenha uma hipótese sendo examinada. Na app aparece em o Psicólogo, a Anamnese e pareceres de gate.

## Como usar

```tsx
// O componente vem do bundle: window.BraboDS.HypothesisIcon

// Ao lado de um rótulo — decorativo, então esconda do leitor de tela.
<button>
  <HypothesisIcon size={14} aria-hidden />
  Continuar
</button>

// Sozinho e significativo — dê a ele um nome acessível.
<HypothesisIcon size={16} aria-label="uma hipótese sendo examinada" />
```
