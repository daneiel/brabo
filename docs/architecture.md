...
Decisions and their rationale live in the [ADRs](adr/index.md) — 106 of
...
**The `ActionType` union is a copy, and a copy ages.** The canonical list is
`ACTION_TYPES` in `apps/api/src/domain/actions/decide.ts`, and `apps/api` is
not a dependency of `apps/web` — the compiler has no way to catch the
divergence. It's already happened twice in production (the three Gitflow
bootstrap types; later `parallelize`/`raise_max_parallel`). Who catches it
today is `lib/aprovacoes.test.ts`, which **reads `decide.ts`** and fails a
type with no sentence; and, when in doubt, the UI degrades instead of
...
| Images aren't published to a registry; the production overlay points at `ghcr.io/OWNER/*` | [ADR 0027](adr/0027-fase5-backup-hardening-release.md) | production deploy isn't executable end to end |
